...
import { fetchEventTypes } from "@/lib/api/analytics";
...
import type { CanonicalEventTypeAggregateResponse } from "@/types/analytics";
import {
  Alert,
  Badge,
  Button,
...
export default function EventTypesPage() {
  ...
  const [data, setData] = useState<EventTypesPageState | null>(null);
    ...
    async function load() {
      ...
      try {
        const [me, eventTypes] = await Promise.all([
          fetchMe(accessToken),
          fetchEventTypes(accessToken, params.organizationId, appliedRange.from, appliedRange.to),
        ]);
