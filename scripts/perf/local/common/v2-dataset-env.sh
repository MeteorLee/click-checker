...
V2_LOCAL_DATASET_VERSION_DEFAULT="${V2_LOCAL_DATASET_VERSION_DEFAULT:-${V2_DATASET_VERSION_DEFAULT}}"
V2_LOCAL_DATASET_DIR_DEFAULT="${V2_LOCAL_DATASET_DIR_DEFAULT:-artifacts/perf/local/common/datasets/${V2_LOCAL_DATASET_VERSION_DEFAULT}}"
V2_LOCAL_DATASET_META_PATH_DEFAULT="${V2_LOCAL_DATASET_META_PATH_DEFAULT:-${V2_LOCAL_DATASET_DIR_DEFAULT}/dataset.json}"
V2_LOCAL_ORG_NAME_PREFIX_DEFAULT="${V2_LOCAL_ORG_NAME_PREFIX_DEFAULT:-perf-v2-local}"

v2_require_local_prepare_url() {
...
v2_local_dataset_dir() {
  printf '%s\n' "${DATASET_DIR:-${V2_LOCAL_DATASET_DIR_DEFAULT}}"
}

v2_local_dataset_meta_path() {
  printf '%s\n' "${DATASET_META_PATH:-${V2_LOCAL_DATASET_META_PATH_DEFAULT}}"
}

v2_local_dataset_exists() {
  [[ -f "$(v2_local_dataset_meta_path)" ]]
}
...
ensure_v2_dataset_local() {
  ...
  if v2_local_dataset_exists; then
    ...
    if ! PREPARE_BASE_URL="${PREPARE_BASE_URL}" \
      DATASET_VERSION="${DATASET_VERSION:-${V2_LOCAL_DATASET_VERSION_DEFAULT}}" \
      DATASET_DIR="$(v2_local_dataset_dir)" \
      DATASET_META_PATH="${dataset_meta_path}" \
      ORG_NAME_PREFIX="${ORG_NAME_PREFIX:-${V2_LOCAL_ORG_NAME_PREFIX_DEFAULT}}" \
