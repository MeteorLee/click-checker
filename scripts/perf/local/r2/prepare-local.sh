...
RATE="${RATE:-10}"
RUN_ID="${RUN_ID:-r2-$(date +%Y%m%d-%H%M%S)-r${RATE}}"
OUT_DIR="${OUT_DIR:-artifacts/perf/local/r2/${RUN_ID}}"
...
QUERY_PRESET="${QUERY_PRESET:-r2-routes-7d-top10}"
TOP="${TOP:-10}"
...
validate_top() {
  ...
  if (( TOP < 1 || TOP > 100 )); then
    echo "TOP must be between 1 and 100" >&2
    exit 1
  fi
}
...
main() {
  ...
  jq -nc \
    ...
    --arg requestEndpoint "/api/v1/events/analytics/aggregates/routes" \
    --arg from "$(jq -r '.rangeFrom' "${DATASET_META_PATH}")" \
    --arg to "$(jq -r '.rangeTo' "${DATASET_META_PATH}")" \
  ...
  echo "[r2-prepare] meta ready: ${meta_path}"
}
