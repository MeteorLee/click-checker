...
OUT_DIR="${OUT_DIR:-artifacts/perf/prod-direct/m1/${RUN_ID}}"
DATASET_VERSION="${DATASET_VERSION:-m1-v1}"
DATASET_DIR="${DATASET_DIR:-artifacts/perf/prod-direct/m1/datasets/${DATASET_VERSION}}"
DATASET_META_PATH="${DATASET_META_PATH:-${DATASET_DIR}/dataset.json}"
DATASET_ORG_NAME="${DATASET_ORG_NAME:-perf-m1-prod-direct-dataset}"
...
create_dataset_if_needed() {
  ...
  "${SCRIPT_DIR}/seed-dataset.sh"
}

restore_snapshot() {
  DATASET_META_PATH="${DATASET_META_PATH}" "${SCRIPT_DIR}/restore-snapshot.sh"
}
