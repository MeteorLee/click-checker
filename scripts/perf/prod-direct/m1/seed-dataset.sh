...
ORG_ID="${ORG_ID:-}"
ORG_NAME="${ORG_NAME:-perf-m1-dataset}"
API_KEY="${API_KEY:-}"
...
main() {
  ...
  echo "[m1-seed] create route templates"
  create_route_templates

  echo "[m1-seed] create event type mappings"
  create_event_type_mappings
...
SQL
  ...
  echo "[m1-seed] seed users and events"
  ...
  trap - EXIT INT TERM
  echo "[m1-seed] dataset ready: ${DATASET_META_PATH}"
}
