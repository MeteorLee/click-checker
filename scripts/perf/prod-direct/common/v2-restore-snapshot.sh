...
verify_v2_dataset_rds() {
  ...
  if [[ "${reset_mode}" == "quick" ]]; then
    [[ "$(query_v2_rds "select count(*) from users where organization_id in (${org_ids_csv});")" -ge "${expected_total_users}" ]] || {
      echo "v2 prod-direct verify failed: total user count fell below baseline after quick reset" >&2
      exit 1
    }
  else
  ...
  if [[ "${reset_mode}" == "quick" ]]; then
    while IFS='|' read -r org_id expected_count; do
      ...
      [[ -n "${actual_count}" && "${actual_count}" -ge "${expected_count}" ]] || {
        echo "v2 prod-direct verify failed: org user count fell below baseline after quick reset" >&2
...
main() {
  ...
  if [[ -z "${DATASET_META_PATH}" ]]; then
    echo "Usage: DATASET_META_PATH=<dataset.json> scripts/perf/prod-direct/common/v2-restore-snapshot.sh" >&2
    exit 1
  fi
  ...
  apply_v2_sql_rds "${sql_path}"
  verify_v2_dataset_rds "${DATASET_META_PATH}" "${RESET_MODE}"
  v2_mark_reset_state_clean "${DATASET_META_PATH}" "reset" "${RESET_MODE}" "${RESET_MODE}"
  echo "[v2-restore-prod-direct] ${RESET_MODE} complete: ${sql_path}"
}
