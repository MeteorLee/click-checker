...
- 이후 운영 디렉터리를 한 번 비우고, 서버 전용 파일(`.env`, backup dump)만 백업한 뒤 CodeDeploy 배포로 다시 채우는 방식으로 정리했다.
- 정리 후 `/home/ubuntu/click-checker`에는 `.git`이 남아 있지 않으며, 현재는 git 작업 트리가 아닌 순수 배포 디렉터리로 사용한다.
...
- 실제로 운영 디렉터리를 재정리한 뒤에는 `git status`가 아니라 `fatal: not a git repository`가 나오는 상태가 됐고, 이 상태가 현재 의도와 맞다.
...
- EC2의 `/home/ubuntu/click-checker`는 현재 `.git` 없는 배포 디렉터리로 유지한다.
