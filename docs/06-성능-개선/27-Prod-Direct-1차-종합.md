...
이번 prod direct 단계에서 실제로 확인한 항목은 아래 일곱 개다.
...
`mixed`는 아직 `30/30`까지도 안정 구간으로 남았다.
...
- local에선 `M1`이 `10/10과 20/20 사이`에서 한계였지만, prod-direct에선 `30/30`도 아직 안정 구간이었다.
...
- `M1 mixed`는 prod-direct에서 local보다 훨씬 높은 pair rate까지 안정적이었다.
...
이건 mixed 실제 한계 위치를 더 좁히고 싶을 때 가는 단계다.
...
> prod-direct에서는 `W1 r100`, `R1 r10~30`, `M1 10/10~30/30`이 모두 안정 구간으로 확인됐고, `R1 overview` 한계 구간은 `30과 50 사이`로 올라갔다. 즉 local 병목 지도는 방향성은 맞았지만, 실제 인프라 여유는 더 크다.
