...
> `EventUser` 경로 최적화, payload text 전환, resolver metadata cache까지 적용한 뒤에도 `M2 s2`는 여전히 fail이다. 이제 다음 우선 병목은 `W2`보다 `M2` realistic mixed의 집계/분석 쿼리 본체와 shared DB work 쪽에 더 가깝다.
