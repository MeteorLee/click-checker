# 2x. 관리자 대시보드 집계 API 계획 (유예, v1.1)

> 현재 이 단계는 바로 구현하지 않는다.
> 9단계 대규모 운영 대응과 집계 경로 최적화 이후에 다시 착수한다.
...
## 1. 이 문서는 "후속 콘솔용 읽기 API" 계획이다
- 이 문서의 핵심은 브라우저 콘솔이 쓸 수 있는 JWT 기반 집계 API를 나중에 어떤 방식으로 만들지 정리하는 것이다.
- 기존 analytics 계산 로직을 다시 설계하거나, 화면 전체를 한 번에 완성하는 단계로 보지 않는다.
- 즉 이 문서는 "9단계 이후에 다시 여는 콘솔용 집계 진입점" 계획이다.
...
## 5. 이후 단계에서는 기존 집계 기능을 콘솔 경로로 다시 연다
...
- 일부 집계 API는 콘솔로 그대로 올리기 전에 9단계 수준의 구조 정리가 더 필요하다.
...
즉 이 문서는 "기존 집계 계산을 다시 만드는 단계"가 아니라, "9단계 이후 JWT 기반 콘솔 진입점을 추가하는 단계"를 위한 보류 계획이다.
...
## 이 문서를 다시 시작하기 전에 먼저 정리되어 있어야 할 항목
...
- route / canonical event type resolver
  - 집계 중 같은 organization 규칙을 반복 조회하지 않도록, 요청 단위 preload 또는 캐시 구조를 먼저 정리한다.
- time-buckets 계열
  - `from`, `to`, `bucket` 조합으로 버킷 수가 과도하게 늘어나는 요청을 제한하는 정책을 먼저 정리한다.
...
## 0) 선행 단계
1. 9단계 대규모 운영 대응
2. 읽기 경로 최적화와 부하 테스트 결과 정리
3. 콘솔에 올려도 될 집계 API 범위 재확정
