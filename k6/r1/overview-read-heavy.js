...
const SCENARIO = __ENV.SCENARIO || 'R1';
const RUN_ID = __ENV.RUN_ID || `r1-manual-${Date.now()}`;
...
export default function () {
  ...
  check(response, {
    ...
    'response has topRoutes': (res) => {
      ...
      try {
        const json = res.json();
        return json && Array.isArray(json.topRoutes);
      } catch (error) {
        return false;
      }
    },
